<?php
$showsoc = "SELECT * FROM role ORDER BY id";
...
<h2>Tabela Role - Primária</h2>
<p>Manutenção das roles no site.</p>
<br>

<a href="./?p=51" class="insert-button">Inserir Role</a>
<a id="delLink" disabled><i class="fa-regular fa-trash-can fa-2xl" onclick="confirmDelete(<?php echo $row['id']; ?>)"></i></a>

<?php
if ($row !== null && isset($row['id'])) {
    $editLink = './?p=55&id=' . $row['id'] . '&operation=editar';
} else {
    $editLink = null;
}
...
<table class="table-hover" style="width:100%; font-size: 20px; margin-top: 1rem; padding-top: 1rem;">
    <thead>
        <tr>
            ...
            <th scope="col" style="text-align: left; width:60%; border-bottom: solid 1px grey; border-collapse: collapse;">Role</th>
        </tr>
    </thead>
    <tbody>
        <?php
...
<script>
    ...
    function enableButtons() {
        ...
        document.getElementById("editLink").href = './?p=55&id=' + encodeURIComponent(selectedID) + '&operation=editar';
    }
    ...
    function confirmDelete() {
        ...
        if (confirm("Tem a certeza?")) {
            var deleteURL = './?p=52&id=' + encodeURIComponent(selectedID) + '&operation=eliminar';
